...
var extracter = function(prefix, suffix, buffer) {
	s = buffer;
...
runner = async () => {
    ...
    const productUrl = "https://off---white.com/";
...
//     cookieJar.setCookie(Cookie.parse(ipp_uid2), "off---white.com");
    ...
    var resp = await cloudscraper.get({
        url: productUrl,  
    ...
    var defaultHeaders = resp.request.headers;
    
    while(resp.body.includes("new Fingerprint2")){
        ...
        const firstvariable = 'decrypt.setPrivateKey("';
        const secondvariable = '");';

        var privatekeystring = extracter(firstvariable,secondvariable, resp.body);
        var decryptdata = extracter('return decrypt.decrypt("', secondvariable, resp.body);
        var decrypt = new jsenc.JSEncrypt();
        decrypt.setPrivateKey(privatekeystring);
        var mydecrypted = crypto.privateDecrypt({key: decrypt.getPrivateKey(), padding: crypto.constants.RSA_PKCS1_PADDING}, Buffer.from(decryptdata,'base64')).toString('utf8');
        
        // var e = crypto.randomBytes(16).toString('hex');
        var e = "2b92dac02651d36268dbdf614a951031";

        var salt = extracter('salt="','";', resp.body);
        ...
        var ipp_key = "ipp_key=" + mydecrypted + "; path=/;";
        var ipp_uid = "ipp_uid=" + extracter('document.cookie="ipp_uid=', ';";', resp.body);
        var ipp_uid1 = "ipp_uid1=" + extracter('document.cookie="ipp_uid1=', ';";', resp.body);
        var ipp_uid2 = "ipp_uid2=" + extracter('document.cookie="ipp_uid2=', ';";', resp.body);
        var ipp_sign = "ipp_sign="+e+"_"+salt+"_"+crypto.createHash('md5').update(e+salt).digest("hex")+"; expires=Tue, 31 Dec 2030 23:59:59 GMT; path=/;";
        
        var Cookie = tough.Cookie;
        console.log(e,salt,mydecrypted);

        // respFlex.forEach(cook => {
        //     cookieJar.setCookie(Cookie.parse(cook), "https://www.off---white.com");
        // });

        // console.log("xoooooxxx", cookieJar);

        cookieJar.setCookie(Cookie.parse(ipp_sign), "https://www.off---white.com");
        cookieJar.setCookie(Cookie.parse(ipp_key), "https://www.off---white.com");
        cookieJar.setCookie(Cookie.parse(ipp_uid), "https://www.off---white.com");
        cookieJar.setCookie(Cookie.parse(ipp_uid1), "https://www.off---white.com");
        cookieJar.setCookie(Cookie.parse(ipp_uid2), "https://www.off---white.com");

        await wait(1000);

        resp = await cloudscraper.get({
            url: productUrl + "?",  
            resolveWithFullResponse: true, 
            simple:false,
            jar:cookieJar,
            ...
            //     'accept-language': 'en,en-GB;q=0.9,en-US;q=0.8,de;q=0.7',
            //     'Connection': 'keep-alive',
            //     Cookie: cookieJar.getCookieString("https://www.off---white.com")
            // },
        })

        // console.log(ipp_sign);

        // console.log(ipp_key);
        // console.log(ipp_uid);
        // console.log(ipp_uid1);
        // console.log(ipp_uid2);
